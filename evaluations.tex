...
As in the qualitative study, most participants appreciated and took advantage of the ability to edit their messages. They found the interface intuitive and natural, presumably due to familiarity with text-editing interfaces. A few participants mistakenly used the Delete key on tokens to fix transcription errors, resulting in the permanent deletion of the audio token. However, the emphasis in the tutorial that the Delete key deleted the audio permanently did help other participants avoid making this mistake.

Usage of editing varied widely: on average, users made about 17.7 edits to each comment (\textit{SD} = 17.5, including inserting a new recording, inserting a pause, deleting words, or deleting a pause). 
...
Given that the F-scores of SimpleSpeech messages were roughly normally distributed and not heavily affected by the experimental conditions, the average F-score of 54.8 (\textit{SD} = 7.2) is likely to be characteristic of general AAC discussions under similar conditions.
