...
The participants were interviewed at the end of the test; these interviews were transcribed, conversational elements filtered out, and the remaining sentences analyzed via two-step coding (open coding followed by flat coding). \red{Cohen's $\kappa$ was .78 indicating the high Inter-coder reliability.}
...
\red{This section is quite hard to understand how you design the study. Please indicate in which demension you were with-in or between subject. Also, do not forget to mention that there was a repetition of a measure--2 sessions for each condition}
...
\subsection{Results}
\red{We could understand effects of the live voice editing features by analyzing system logs, speech contents, and the task load survey data. The results fell into the following three categories.}

\subsubsection{Utilization of Editing Features}
As in the qualitative study, most participants appreciated and took advantage of the ability to edit their messages. \red{They found the interface intuitive and natural thanks to the familiarity with the text-editing interfaces. A few participants mistakenly hit the Delete key on the token when they wanted to fix a transcription error, resulting in the permanent deletion of the audio token. However, The emphasis in the tutorial that the Delete key deleted the audio permanently did help other participants avoid making this mistake.} 
...
\red{IMPORTANT!. Always report standard deviation for each statistics. Also report the mean down to the first place of decimal. The formal way to do this is: On average, users made about 17.X edits to each comments (SD = XX.X, inserting a new ....).\{...\} 7.X word deletions (SD = X.X) and 6.3 pause deletions per message (SD = X.X). }

\red{How many insertions?}
