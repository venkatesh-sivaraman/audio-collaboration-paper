...
\red{The length of speech were about 1 min (M = 57.2, SD = 31.2) per session. The mean word counts of speech per session was 122.3 (SD = 59.1). There was no statistically significant difference of the speech length or word counts by different conditions.}
...
Overall, the students reported significantly \emph{lower} levels of mental task load or pressure during the E task than the NE task (\red{M$_{E}$ = 8.7, SD$_{E}$ = X.X compared to M$_{NE}$ = 10.8, SD$_{NE}$ = X.X}, $p<0.02$ using a two-tailed $t$-test). 
